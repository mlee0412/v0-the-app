...
class MenuDataService {
  ...
  private async processPdfMenu(fileName: string): Promise<void> {
    ...
    try {
      ...
      // Use eval to avoid bundling this heavy Node-only dependency in client builds
      const { default: pdfParse } = await eval("import('pdf-parse')") as any;
  ...
  private async processExcelMenu(fileName: string): Promise<void> {
    ...
    try {
      ...
      const buffer = Buffer.from(await data.arrayBuffer());
      // Dynamically import server-only Excel parser
      const XLSX = await eval("import('xlsx')") as any;
