...
import { type UserRole, type User, type Permission, ADMIN_LEVEL_ROLES } from "@/types/user"
...
class SupabaseAuthService {
  ...
  async getUsers(): Promise<User[]> {
    try {
      ...
      const response = await fetch(`/api/staff/members?t=${timestamp}`)
      ...
      if (!contentType || !contentType.includes("application/json")) {
        ...
        // Return default admin user when API returns non-JSON response
      ...
      const formattedUsers: User[] = (data || []).map((user: any) => ({
        ...
        username:
          user.username ||
          user.email ||
          (user.first_name ? user.first_name.toLowerCase().replace(/\s+/g, "") : undefined),
      ...
      if (
        !formattedUsers.some((u) => u.role === "admin" && (u.email === "admin@example.com" || u.username === "admin"))
      ) {
        formattedUsers.push(this.createDefaultAdminUser())
      }
  ...
  // Helper method to get default users when API fails
  private getDefaultUsers(): User[] {
    console.log("Using default users due to API error")
  ...
  async addUser(userData: Partial<User>): Promise<User | null> {
    // Use Partial<User> for input
    try {
  ...
  async updateUserPermissions(userId: string, permissions: Partial<Permission>): Promise<Permission | null> {
    // Use Partial<Permission>
    try {
      const response = await fetch(`/api/staff/permissions/${userId}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async logout() {
    // Kept async for Supabase signout
    if (this.isBrowser) {
      localStorage.removeItem("currentUser")
    }
