...
const TableTimerDisplayComponent = ({
  ...
  // Update timer only when active - using RAF for smooth updates
  useEffect(() => {
    if (!table.isActive || !table.startTime) {
      // For inactive tables, just set to initial time
      setDisplayTime(table.initialTime);
      return;
    }

    // Active table - use RAF for smooth updates
    const updateTimer = () => {
      const now = Date.now();

      // Only update display every ~100ms to reduce re-renders while staying smooth
  ...
  }, [table.id, table.isActive, table.startTime, table.initialTime]);

  // Sync with external updates (from props changes)
  useEffect(() => {
    if (!table.isActive || !table.startTime) {
      setDisplayTime(table.initialTime);
    }
  }, [table.id, table.isActive, table.startTime, table.initialTime, table.remainingTime]);
