...
const TableCardComponent = function TableCard({
  ...
  const calculateStatus = useCallback(
    (remainingTime: number) => {
      ...
      const isOvertime = isActive && remainingTime < 0
      const isWarningYellow = isActive && remainingTime <= fifteenMinutesInMs && remainingTime > tenMinutesInMs
      const isWarningOrange =
        isActive && remainingTime <= tenMinutesInMs && remainingTime > criticalThresholdMs
      const isCritical = isActive && remainingTime <= criticalThresholdMs && remainingTime >= 0

      const orangeIntensity =
        isWarningOrange && tenMinutesInMs - criticalThresholdMs > 0
          ? 1 - (remainingTime - criticalThresholdMs) / (tenMinutesInMs - criticalThresholdMs)
          : 0

      const criticalIntensity =
        isCritical && criticalThresholdMs > 0 ? 1 - remainingTime / criticalThresholdMs : 0

      const shouldAnimateWarning = showAnimations && isActive && remainingTime <= fifteenMinutesInMs
  ...
  const [tableStatus, setTableStatus] = useState(() => calculateStatus(computeRemainingTime()))

  useEffect(() => {
    const updateStatus = () => {
      const nextStatus = calculateStatus(computeRemainingTime())

      setTableStatus((prev) => {
        if (!prev) return nextStatus

        const hasSignificantTimeChange = Math.abs(prev.remainingTime - nextStatus.remainingTime) >= 1000

        if (
          !hasSignificantTimeChange &&
          prev.isOvertime === nextStatus.isOvertime &&
          prev.isWarningYellow === nextStatus.isWarningYellow &&
          prev.isWarningOrange === nextStatus.isWarningOrange &&
          prev.isCritical === nextStatus.isCritical &&
          prev.shouldAnimateWarning === nextStatus.shouldAnimateWarning
        ) {
          return prev
        }
    ...
    return () => {
      if (statusRafRef.current !== null) {
        cancelAnimationFrame(statusRafRef.current)
        statusRafRef.current = null
      }
    }
  }, [calculateStatus, computeRemainingTime, localTable.isActive, localTable.startTime])
  ...
  }, [localTable.groupId, localTable.isActive, getGroupColor, showAnimations, tableStatus])
  ...
  const backgroundStyles = useMemo(() => {
    ...
    return { style, animationClassName }
  }, [localTable.isActive, showAnimations, tableStatus])
  ...
  const formattedRemainingTime = useMemo(() => {
    const value = tableStatus.remainingTime
    const isNegative = value < 0
    const absoluteMs = Math.abs(value)
